import {
	IWeatherCardAdditionalData,
	IWeatherCardTemperature,
	IWeatherCardWeatherListItem,
} from "@/interfaces/IWeatherCard";
...
export function createTemperatrure(temperature: any): IWeatherCardTemperature {
	return { value: Math.round(temperature.temp * 10) / 10, unit: "°C" };
}

export function createWeatherList(
	weather: Array<any>
): Array<IWeatherCardWeatherListItem> {
	return weather.map(({ description, icon }) => {
		return { description, icon: convertIco(icon) };
	});
}

export function сreateDescription({
	feelsLike,
	weather,
}: IDescriptionCardData): string {
	return (
		`Feels like ${feelsLike} ` +
		weather.map((e) => upperFirstLetter(e.description)).join(". ")
	);
}
...
export function createDevPoint(
	temp: any,
	pressure: any
): IWeatherCardAdditionalData {
	return {
